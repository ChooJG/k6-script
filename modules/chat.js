...
export function startChat(token) {
    ...
    if (success) {
        try {
            ...
            console.log(`✅ 채팅 시작: 스토리 ID ${storyId}`);
            return storyId;
        } catch (e) {
...
export function setGenre(token, storyId) {
    ...
    const genre = TEST_DATA.genres[Math.floor(Math.random() * TEST_DATA.genres.length)];
...
export function setTheme(token, storyId) {
    ...
    const theme = TEST_DATA.themes[Math.floor(Math.random() * TEST_DATA.themes.length)];
...
export function sendMessage(token, storyId) {
    ...
    const message = TEST_DATA.messages[Math.floor(Math.random() * TEST_DATA.messages.length)];
